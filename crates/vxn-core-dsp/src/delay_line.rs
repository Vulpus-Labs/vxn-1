...
//!
//! Moved here from `vxn-dsp` by ticket 0229, following its only consumer:
//! [`crate::chorus`]. What remains is the modulated delay line
//! and the support types it needs ([`DelayBuffer`] cubic/Thiran reads,
//! [`BoundedRandomWalk`] clock jitter, [`OnePoleLpf`]).
//!
//! The chorus only needs the *short-delay* BBD regime that chorus lives in
...
use vxn_core_utils::random_walk::BoundedRandomWalk;
