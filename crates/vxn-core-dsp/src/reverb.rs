//! 8-channel FDN reverb (ADR §7) — the shared kernel.
//!
//! Moved here from `vxn2-dsp` by ticket 0230 (epic E041). vxn-1b's copy was
//! the same topology, `BASE_MS` table and LFO scheme; the only real difference
//! was that its bypass was owned by an outer crossfade rather than an internal
//! fade. The mix law was already identical in both — see `reverb_mix` below.
//!
...
//! - `reverb_mix` is the wet/dry crossfade, **equal-power**:
//!   `√(1-mix)·dry + √mix·wet`. The wet is decorrelated from the dry, so the
//!   two sum in power rather than amplitude and sqrt gains hold total power
//!   constant across the sweep. (Linear gains, which this line claimed until
//!   ticket 0230, dip ~3 dB at mix = 0.5; the code went equal-power in
//!   5460922, 2026-06-22, and the doc was never updated.)
//!
//! ## Stereo image
//!
...
use vxn_core_utils::math::fast_sine_01;
...
pub struct FdnReverb {
    ...
    /// zipper, fades in on switch-on, fades out rather than cutting).
    fade: WetFade,
}
...
impl FxKernel for FdnReverb {
    type Params = FdnReverbParams;
    ...
    fn new(sample_rate: f32) -> Self {
        ...
        let mut r = Self {
            ...
            damp_a: 0.0,
            feedback: 0.0,
            fade: WetFade::new(MIX_SMOOTH_MS, sr),
        };
    ...
    fn set_params(&mut self, p: &FdnReverbParams) {
        ...
        // so a patch loaded with the reverb already set does not ride in.
    ...
    fn process(&mut self, in_l: f32, in_r: f32) -> (f32, f32) {
        ...
        for i in 0..LINES {
            ...
            // the operator core.
            let s = fast_sine_01(self.lfo_phase[i]);
    ...
    /// Zero buffers + filter state; smoother targets preserved. This is the
    /// stale-state drop, not a re-idle — see [`FxKernel::reset`].
    fn clear(&mut self) {
        self.ring.clear();
    ...
    /// and settle the fade to whatever the enable flag currently says.
    fn reset(&mut self) {
        self.clear();
        self.fade.reset();
    }
    ...
    // `state_abs_max` keeps the conservative default. A real figure would
    // have to scan the whole 8-line ring, not just `damp_y`, and a partial
    // measure here would license a caller to skip a span while the tail is
    // still ringing — worse than not overriding it.
}
