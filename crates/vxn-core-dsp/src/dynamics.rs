...
//!
//! The kernel is rate-agnostic — one stereo sample in / out — so it carries no
//! oversampling stage: in vxn-2 the 4x interp/decimate lives in the engine, not
//! here, and VXN1b runs it at whatever the global OS rate is.
//!
//! **Provenance** (0227). Written for vxn-2, ported verbatim into vxn-1 by
//! E037/0206, then the two drifted: ticket 0241 added the gain-reduction
//! metering tap to vxn-1's copy only. This is that copy — the superset — and
//! both crates now re-export it. The metering is a side channel (`gr_db_min` is
//! never read back into the output), so adopting it leaves vxn-2's audio
//! bit-identical; it simply gains a tap it can publish whenever it wants one.

use vxn_core_utils::math::fast_tanh;
use vxn_core_utils::smoothing::{Smoothed, one_pole_coeff};
...
mod tests {
    ...
    use crate::test_util::{assert_bit_exact_after_settle, assert_bit_exact_passthrough};
    ...
    fn off_from_load_is_bit_exact_from_first_sample() {
        ...
        assert_bit_exact_passthrough(|x, y| d.process(x, y), 1_000);
    }
    ...
    fn switch_off_fades_then_settles_to_bit_exact() {
        ...
        assert_bit_exact_after_settle(|x, y| d.process(x, y), settle, 1_000);
    }
