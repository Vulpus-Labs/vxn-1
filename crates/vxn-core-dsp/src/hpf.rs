//! One-pole (−6 dB/oct) high-pass filter — the scalar kernel, shared.
//!
//! In vxn-2 it sits pre-VCF in the voice-stack chain (stack sum → HPF → musical
//! filter → FX), thinning body and removing DC / low rumble below the cutoff.
//! It runs on a stack's summed stereo pair, so it needs two scalar kernels per
//! stack rather than the SoA poly form.
//!
//! **Provenance** (0227). vxn-1 and vxn-2 had identical bodies with different
//! status: vxn-2's ships, while vxn-1's was `#[cfg(test)]`-only, existing purely
//! as the scalar oracle its 8-wide `PolyHpf` is differentially tested against.
//! One kernel now serves both roles. `PolyHpf` itself stays in `vxn-dsp` — it is
//! an SoA lane body, which ADR 0002 §3 keeps per-synth.
//!
...
///
/// Public because vxn-1's 8-wide `PolyHpf` needs the *same* mapping while
/// keeping its own SoA body: sharing the kernel but not this would leave the
/// scalar oracle and the lane body free to disagree on the one thing their
/// differential test assumes they agree on.
#[inline]
pub fn coeff(cutoff_hz: f32, sample_rate: f32) -> f32 {
