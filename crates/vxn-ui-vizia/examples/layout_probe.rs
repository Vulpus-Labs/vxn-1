...
//!
//! Wrap views of interest in `vxn_ui_vizia::Probe::new(cx, "name", |cx| ...)` inside
//! the editor build code, then:
//!
//! ```text
//! cargo run -p vxn-ui-vizia --example layout_probe --features layout-probe \
//!   2>&1 | grep PROBE | sort -u
//! ```
//!
//! The window renders off-screen here, so no Screen Recording / window-server
//! permission is needed — the bounds are printed to stderr. See `vxn_ui_vizia::Probe`.

#[cfg(feature = "layout-probe")]
fn main() {
    vxn_ui_vizia::run_layout_probe();
}
