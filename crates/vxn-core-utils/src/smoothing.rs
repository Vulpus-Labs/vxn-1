...
/// zero** — a zero-length window is meaningful (the limiter's lookahead
/// legitimately goes to 0 at its minimum attack).
///
/// A rounding, floor-1 sibling (`fade_len_samples`) existed for vxn-1's fade
/// windows and was removed with it (2026-08-27, vxn-1 retirement). Nothing
/// needs the distinction now: its only caller was vxn-1's engine, and the
/// degenerate-window hazard it guarded was already covered by
/// `BypassXfade::new`'s own clamp — a type that had no other consumers and went
/// with it.
#[inline]
