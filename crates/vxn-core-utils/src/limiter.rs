...
//!
//! Shared by both synths (E027/0118) — `vxn-dsp` and `vxn2-dsp` had
//! byte-identical copies. Optional brickwall on the master bus — the engine
//! runs it last in the FX chain (after master gain) when the `limiter-on`
//! patch flag is set, bypassed otherwise.
//!
...
use crate::smoothing::{ms_to_samples, one_pole_coeff};

// ── Power-of-two delay line ──────────────────────────────────────────────────

/// Power-of-two circular buffer with fractional (linear-interpolated) reads.
/// Private to the limiter (mirrors VXN1's `vxn-dsp::delay::DelayLine`; the
/// synths' public delay rings aren't shared here).
struct DelayLine {
    buf: Vec<f32>,
    mask: usize,
    write: usize,
