...
This is a **prerequisite epic**: E010 (standalone mac + windows) needs a
...
- The standalone executables (mac + windows) — that is E010, which
...
- macOS-universal / Windows **release** artifacts in `release.yml` —
  optional stretch (ticket 0102); the core deliverable is the
  `build-windows.yml` CI artifact.

## Tickets

- [ ] [0098 — xtask: cross-platform bundle (Win/Linux renamed lib → .clap)](../../tickets/open/0098-xtask-cross-platform-bundle.md)
- [ ] [0099 — vxn-2 compiles clean under MSVC on windows-latest](../../tickets/open/0099-vxn2-windows-compile.md)
- [ ] [0100 — build-windows.yml CI workflow for vxn-2](../../tickets/open/0100-build-windows-ci.md)
- [ ] [0101 — Windows editor / window-handling verification](../../tickets/open/0101-windows-editor-verify.md)
- [ ] [0102 — release.yml vxn-2 artifacts (mac-universal + win-x64)](../../tickets/open/0102-release-vxn2-artifacts.md) *(stretch)*

## Dependency order

```text
0098 (xtask cross-platform bundle) ──┐
                                      ├─> 0100 (build-windows CI) ─> 0101 (editor verify) ─> 0102 (release)
0099 (Windows compile fixes) ────────┘
```

- 0098 and 0099 are independent and can land in parallel; 0098 is pure
  xtask logic, 0099 is whatever the first MSVC build surfaces.
- 0100 needs both: a cross-platform bundle step AND a workspace that
  compiles on Windows.
- 0101 needs a built `VXN2.clap` (from 0100's artifact or a local
  Windows build) and a Windows host.
- 0102 is a stretch packaging step that can land any time after 0100.
...
- **First MSVC compile is an unknown.** vxn-2 has only ever built on
  ...
  assumption in vxn2-* will break. Budget 0099 as discovery.
- **"Windows no-UI" bug class.** vxn-1's `gui.rs` documents that
  ...
  has the win32 branch, but it has never executed — 0101 must confirm
...
- In a Windows CLAP host: the plugin loads, the WebView2 editor opens
  ...
  text-input popup accepts Enter/Esc. Recorded in 0101 with the
