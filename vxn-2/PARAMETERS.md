...
| `level`         | i    | 0 .. 99        | 99 (carriers) / 0 (mods) | Operator output level. For carriers this is amp; for modulators this drives FM index. DX7's primary timbre control. **Not a linear fader**: above the knee at 20 the scale is ~0.75 dB per step, `amp = 2^((level-99)/8)`, so the 0..99 range spans ~74 dB and level 70 is -21.8 dB, not 70 %. Halving amplitude is -8 level units. |
...
| `eg_l1..l4`     | i    | 0 .. 99        | 99,70,50,0  | EG levels. L1 = peak after attack, L2 = decay target, L3 = sustain level, L4 = release floor (and pre-attack start). Carriers usually L4=0. Same ~0.75 dB/step law as `level`, and the two **add** in that domain rather than acting independently - the effective sustained output of an operator is `level + eg_l3 - 99`. A modulator at `level 74` with `eg_l3 66` sustains at 41, which is an FM index of 0.04: a sine. See [ADR 0010](adrs/0010-log-domain-level-pipeline.md). |
