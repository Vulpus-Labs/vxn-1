// Browser-storage primitive for user presets (E030 / 0159).
//
// The web port replaces the desktop std::fs user-preset side with IndexedDB:
// the corpus is small key→value blobs, IndexedDB is universal, and a flat store
// fits it well. This module is JUST the storage layer: open the DB and read /
// write the three object stores. It is intentionally dumb — no corpus logic, no
// controller coupling. The wasm-side UserState (user_store.rs) owns the cache +
// the write journal; the bridge (preset-persistence.mjs) hydrates the cache
// from getAll() at boot and flushes the journal here via applyWrites().
//
//   store "presets": key = synthetic path ("folder/Name.toml" | "Name.toml"),
//                    value = Uint8Array (the preset's TOML record bytes — the
//                    same name-keyed TOML the desktop plugin writes).
...
export const DB_NAME = "vxn2-presets";
...
// All presets as [{ key, bytes }] (bytes = Uint8Array). Used by boot hydration
// to seed the wasm cache.
...
// Full patch-state autosave slot. One key→blob entry, the host-state analogue.
// getState resolves the stored Uint8Array or null. The `get`'s onsuccess fires
// before the transaction's oncomplete (which resolves tx), so the captured
// result is ready by then — same shape as getAllPresets' cursor accumulation.
...
// 'put_folder'|'delete_folder', key?, bytes?, name?}. Ops run sequentially; a
// failure rejects so the caller can surface a storage error (quota/eviction).
