...
//!
//! [`alloc::PolyAlloc`] — 16-voice polyphony with oldest-note stealing,
//! Poly / Solo assignment, glide, and channel-wide pitch bend. Higher-level
//! stacking, modulation, voicing modes, and FX layer on top.
...
// FTZ guard shared with vxn-1. Re-exported so vxn2-clap and the engine's
// `process` boundary keep importing `ScopedFlushToZero` from here.
...
/// display path. Canonical source lives in `vxn2-dsp::lfo`.
