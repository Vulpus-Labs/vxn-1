...
macro_rules! op_block_arr {
    ($prefix:literal, $n:literal) => {
        [
            fl(concat!($prefix, "op", $n, "-ratio"), concat!("Op ", $n, " Ratio"), 0.5, 31.0, 1.0, ""),
            flx(concat!($prefix, "op", $n, "-fixed-hz"), concat!("Op ", $n, " Fixed Hz"), 1.0, 9772.0, 440.0, "Hz", 100.0),
            fl(concat!($prefix, "op", $n, "-fine"), concat!("Op ", $n, " Fine"), 0.0, 0.99, 0.0, ""),
            it(concat!($prefix, "op", $n, "-detune"), concat!("Op ", $n, " Detune"), -7, 7, 0, ""),
            it(concat!($prefix, "op", $n, "-level"), concat!("Op ", $n, " Level"), 0, 99, 99, ""),
            it(concat!($prefix, "op", $n, "-vel-sens"), concat!("Op ", $n, " Vel Sens"), 0, 7, 3, ""),
            it(concat!($prefix, "op", $n, "-amp-sens"), concat!("Op ", $n, " Amp Sens"), 0, 3, 0, ""),
            it(concat!($prefix, "op", $n, "-eg-r1"), concat!("Op ", $n, " EG R1"), 0, 99, 99, ""),
            it(concat!($prefix, "op", $n, "-eg-r2"), concat!("Op ", $n, " EG R2"), 0, 99, 50, ""),
            it(concat!($prefix, "op", $n, "-eg-r3"), concat!("Op ", $n, " EG R3"), 0, 99, 35, ""),
            it(concat!($prefix, "op", $n, "-eg-r4"), concat!("Op ", $n, " EG R4"), 0, 99, 60, ""),
            it(concat!($prefix, "op", $n, "-eg-l1"), concat!("Op ", $n, " EG L1"), 0, 99, 99, ""),
            it(concat!($prefix, "op", $n, "-eg-l2"), concat!("Op ", $n, " EG L2"), 0, 99, 70, ""),
            it(concat!($prefix, "op", $n, "-eg-l3"), concat!("Op ", $n, " EG L3"), 0, 99, 50, ""),
            it(concat!($prefix, "op", $n, "-eg-l4"), concat!("Op ", $n, " EG L4"), 0, 99, 0, ""),
            it(concat!($prefix, "op", $n, "-ks-break-pt"), concat!("Op ", $n, " KS Break"), 0, 127, 60, ""),
            it(concat!($prefix, "op", $n, "-ks-l-depth"), concat!("Op ", $n, " KS L Depth"), 0, 99, 0, ""),
            it(concat!($prefix, "op", $n, "-ks-r-depth"), concat!("Op ", $n, " KS R Depth"), 0, 99, 30, ""),
            it(concat!($prefix, "op", $n, "-ks-rate"), concat!("Op ", $n, " KS Rate"), 0, 7, 2, ""),
            fl(concat!($prefix, "op", $n, "-pan"), concat!("Op ", $n, " Pan"), -1.0, 1.0, 0.0, ""),
            it(concat!($prefix, "op", $n, "-feedback"), concat!("Op ", $n, " Feedback"), 0, 7, 0, ""),
        ]
    };
}
...
macro_rules! per_layer_rest_arr {
    ($prefix:literal) => {
        [
            it(concat!($prefix, "algo"), "Algorithm", 1, 32, 5, ""),
            en(concat!($prefix, "lfo2-shape"), "LFO2 Shape", LFO_SHAPES, 2),
            flx(concat!($prefix, "lfo2-rate"), "LFO2 Rate", 0.01, 50.0, 5.1, "Hz", 2.0),
            flx(concat!($prefix, "lfo2-delay"), "LFO2 Delay", 0.0, 4000.0, 180.0, "ms", 100.0),
            flx(concat!($prefix, "lfo2-fade"), "LFO2 Fade", 0.0, 4000.0, 320.0, "ms", 100.0),
            en(concat!($prefix, "lfo2-trig"), "LFO2 Trig", LFO2_TRIGS, 1),
            it(concat!($prefix, "peg-r1"), "PEG R1", 0, 99, 99, ""),
            it(concat!($prefix, "peg-r2"), "PEG R2", 0, 99, 50, ""),
            it(concat!($prefix, "peg-r3"), "PEG R3", 0, 99, 35, ""),
            it(concat!($prefix, "peg-r4"), "PEG R4", 0, 99, 60, ""),
            it(concat!($prefix, "peg-l1"), "PEG L1", -99, 99, 0, ""),
            it(concat!($prefix, "peg-l2"), "PEG L2", -99, 99, 0, ""),
            it(concat!($prefix, "peg-l3"), "PEG L3", -99, 99, 0, ""),
            it(concat!($prefix, "peg-l4"), "PEG L4", -99, 99, 0, ""),
            fl(concat!($prefix, "peg-depth"), "PEG Depth", 0.0, 1.0, 1.0, ""),
            flx(concat!($prefix, "mod-env-a"), "Mod Env A", 0.0, 4000.0, 2.0, "ms", 50.0),
            flx(concat!($prefix, "mod-env-d"), "Mod Env D", 0.0, 4000.0, 320.0, "ms", 100.0),
            fl(concat!($prefix, "mod-env-s"), "Mod Env S", 0.0, 1.0, 0.60, ""),
            flx(concat!($prefix, "mod-env-r"), "Mod Env R", 0.0, 4000.0, 180.0, "ms", 100.0),
            en(concat!($prefix, "mod-env-shape"), "Mod Env Shape", ADSR_SHAPES, 0),
            en(concat!($prefix, "assign-mode"), "Assign", ASSIGN_MODES, 0),
            bl(concat!($prefix, "legato"), "Legato", false),
            flx(concat!($prefix, "glide-time"), "Glide", 0.0, 2000.0, 12.0, "ms", 100.0),
            it(concat!($prefix, "stack-density"), "Stack Density", 1, 8, 4, ""),
            fl(concat!($prefix, "stack-detune"), "Stack Detune", 0.0, 100.0, 8.0, "ct"),
            fl(concat!($prefix, "stack-spread"), "Stack Spread", 0.0, 1.0, 0.60, ""),
            fl(concat!($prefix, "stack-phase"), "Stack Phase", 0.0, 1.0, 0.50, ""),
            en(concat!($prefix, "stack-distrib"), "Stack Distrib", STACK_DISTRIBS, 0),
            fl(concat!($prefix, "mtx1-depth"), "Mtx 1 Depth", -1.0, 1.0, 0.0, ""),
            fl(concat!($prefix, "mtx2-depth"), "Mtx 2 Depth", -1.0, 1.0, 0.0, ""),
            fl(concat!($prefix, "mtx3-depth"), "Mtx 3 Depth", -1.0, 1.0, 0.0, ""),
            fl(concat!($prefix, "mtx4-depth"), "Mtx 4 Depth", -1.0, 1.0, 0.0, ""),
            fl(concat!($prefix, "mtx5-depth"), "Mtx 5 Depth", -1.0, 1.0, 0.0, ""),
            fl(concat!($prefix, "mtx6-depth"), "Mtx 6 Depth", -1.0, 1.0, 0.0, ""),
            fl(concat!($prefix, "mtx7-depth"), "Mtx 7 Depth", -1.0, 1.0, 0.0, ""),
            fl(concat!($prefix, "mtx8-depth"), "Mtx 8 Depth", -1.0, 1.0, 0.0, ""),
        ]
    };
}
...
const UPPER: [ParamDesc; N_PER_LAYER] = concat_per_layer(
    [
        op_block_arr!("upper-", "1"),
        op_block_arr!("upper-", "2"),
        op_block_arr!("upper-", "3"),
        op_block_arr!("upper-", "4"),
        op_block_arr!("upper-", "5"),
        op_block_arr!("upper-", "6"),
    ],
    per_layer_rest_arr!("upper-"),
);

const LOWER: [ParamDesc; N_PER_LAYER] = concat_per_layer(
    [
        op_block_arr!("lower-", "1"),
        op_block_arr!("lower-", "2"),
        op_block_arr!("lower-", "3"),
        op_block_arr!("lower-", "4"),
        op_block_arr!("lower-", "5"),
        op_block_arr!("lower-", "6"),
    ],
    per_layer_rest_arr!("lower-"),
);
