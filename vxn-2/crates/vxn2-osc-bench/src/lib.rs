//! Sine-generator candidates for the VXN2 operator architecture.
//!
//! Three impls share a Q32 fixed-point phase domain (full u32 range = one
//! cycle, wraparound is free via integer add). The reader differs:
//!
//! - [`scalar::fast_sine_q32`] — Bhaskara I + Moser polynomial. Branch-free,
//!   pure ALU. Max abs err ≈ 0.001 vs `f64::sin`. THD ≈ -59 dB.
//! - [`scalar::lookup_sine_q32`] — 1024-entry table, linear interp, shift+mask
//!   index. Max abs err ≈ 1e-6. THD ≈ -133 dB. Scalar gather → poor SIMD.
...
pub mod scalar {
    ...
    /// Branch-free; ~5 mul + 2 abs + 2 add after the Q32→[0,1) conversion.
    #[inline(always)]
    pub fn fast_sine_q32(phase: u32) -> f32 {
        // Q32 → [0, 1): divide by 2^32.
...
pub mod neon {
    ...
    ///
    /// Computes `fast_sine_q32` on four phases simultaneously. No branches,
    /// no gather — pure register-to-register ALU on NEON.
    #[inline]
...
/// Minimal scalar ADSR for op-level env bench. Linear stages.
pub mod adsr {
    ...
    impl Adsr {
        ...
        /// Force into Sustain at a given level — fixture for steady-state bench.
...
mod tests {
    ...
    fn fast_sine_accuracy() {
        // Sweep one full cycle; assert max abs error vs f64::sin < 2e-3.
