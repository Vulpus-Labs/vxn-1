...
- [ ] Collapse the four builders `pitched_synth` (~1341), `osc2_sine_synth`
...
- [ ] Add `fn loaded_names(rx) -> Vec<String>` for the `drain(&view_rx)...
      filter_map(PresetLoaded => name)` pattern copied ~5× (~842/857/872/
      948/957).

`vxn-clap/tests/host_smoke.rs`:

- [ ] Add `fn load_entry()` for the repeated
      `unsafe { PluginEntry::load_from_raw(&VXN_ENTRY, c"...") }.unwrap()`
      (~31/41).

- [ ] `cargo test -p vxn-engine -p vxn-app -p vxn-clap` green; assertions
      unchanged.
