...
- MIDI channel threaded note-on → allocation → voice, re-parented on steal:
  [voice.rs](../../vxn-1b/crates/vxn1b-engine/src/voice.rs) `Voices` (`channel[]`,
  `note_on`). Per-voice `pressure[]` folded by channel: `poly_pressure` (matching
  channel+note only), `channel_pressure` (broadcast on channel). Exposed via
  `pressure(v)`. Tests: `voice::tests::{per_note_pressure_isolated_to_matching_voice,
  per_note_pressure_ignores_same_note_other_channel, channel_pressure_broadcasts_to_all_voices_on_channel,
  stolen_voice_reparents_to_stealing_channel}`. Allocation-free (fixed arrays;
  `tests/alloc_free.rs::hot_path_is_allocation_free`).
