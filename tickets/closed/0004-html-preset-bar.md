...
button that toggles 0005's browser panel, and a "Save As" action
...
- [ ] Browse button toggles a `browserOpen: bool` view-side state;
      0005 ticket binds the browser panel to it.
...
currently has selected, defaulting to user root. For 0004 (browser
not yet built), default to user root unconditionally; 0006 ties the
two together.
