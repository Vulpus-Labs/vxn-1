...
the `vxn-clap` archive produced by ticket 0008 and producing
`VXN1.vst3` for the host platform. Cross-platform (mac +
win); the Windows-specific xtask wiring lives in ticket 0012,
...
- [ ] Same invocation succeeds on Windows under MSVC 2022 +
      ...
      in ticket 0013; here we only need successful build +
...
`CMakeLists.txt` in the pinned tag (ticket 0009) for the
