...
Companion to vxn-2's E012 (same review, same day). Items
...
- Docs/epic-state sweep: vxn-1 README self-contradiction
  ...
  withdrawal notes, close epics E002/E004/E005/E006/E007/
  E008 whose tickets are all in `closed/`.
...
- Root-level docs (root README workspace claims, root ADR
  0001 status) — covered by vxn-2 E012 ticket 0072.
- vxn-2 findings of the same review — E012.
...
- **0015** Smoothing policy for FX + mixer params.
- **0016** JS orchestration test coverage.
- **0017** LocalParams gesture unification with vxn-core-clap.
- **0018** Docs and epic-state sweep.
- **0019** Rust hygiene batch (engine/dsp/app/xtask).
- **0020** UI-web hygiene batch (JS/CSS/assets).
...
0015 (smoothing)     ── independent
0016 (JS tests)      ── before 0020 (tests pin behaviour
                        the cleanup then must not change)
0017 (LocalParams)   ── coordinate with vxn-2 0065 (gesture
                        port); whoever lands first shapes
                        the shared type
0018 (docs sweep)    ── independent
0019 (rust hygiene)  ── independent
0020 (ui hygiene)    ── after 0016
```
...
- `cargo test --workspace` green at epic close; audio
  ...
  0015 deliberately alters smoothed-param renders, with the
...
never reads (0019 adds the comment that prevents the next
