//! CLAP `gui` extension: embeds the HTML WebView editor into the host's
...
impl PluginGuiImpl for VxnMainThread<'_> {
    ...
    fn get_size(&mut self) -> Option<GuiSize> {
        Some(GuiSize {
            width: vxn_ui_web::EDITOR_WIDTH,
            height: vxn_ui_web::EDITOR_HEIGHT,
        })
    }
    ...
    fn set_parent(&mut self, window: Window) -> Result<(), PluginError> {
        ...
        // Pull the raw pointer per platform; the WebView wraps it in its own
        ...
        // preset-corpus snapshot (0050) — the editor's browser panel re-reads
        // this on every `PresetCorpusChanged`. View-event drain + controller
        // tick are driven from the host's main-thread timer (registered
        // below), not an editor-internal idle hook.
        ...
        // panic across the host's C ABI (0115). The plugin stays alive;
        // the host may retry set_parent.
        self.gui = Some(vxn_ui_web::open_editor(parent, ctrl_handle, corpus)?);
