...
fn web(release: bool, serve: bool, scalar: bool, port: Option<&str>) -> Result<(), String> {
    ...
    const CORE_MODULES: [&str; 6] = [
        // User-preset persistence: the IndexedDB primitive + the async-storage
        // <-> sync-controller bridge faceplate-bridge loads.
        "preset-storage.mjs",
        "preset-persistence.mjs",
        // Full patch-state autosave/restore: the host-state-blob analogue
        // faceplate-bridge restores at boot + debounces writes on edit.
        "state-autosave.mjs",
        // Patch export/import + URL share-link: faceplate-bridge injects the
        // preset-bar controls + applies a `#patch=` link at boot.
        "patch-io.mjs",
        // E017 input adapters: browser input → E015 ring producers.
        "midi-input.mjs",
        "keyboard-input.mjs",
    ];
    const MODULES: [(&str, &str); 12] = [
        ...
        // Key mode / split point (vxn-1 only — no other port has layers, so this
        // one stays local rather than joining the shared set).
    ...
    for m in CORE_MODULES {
        ...
        fs::copy(&from, dist.join(m)).map_err(io("copy shared web module"))?;
    }
