...
//!
//! Ported from `patches-modules::dynamics::StereoLimiter` and its shared
//! `LimiterCore`, rewritten for VXN1's framework-agnostic per-sample model and
//! simplified to sample-peak detection. Used as an optional brickwall on the
//! master bus (the engine runs it last in the FX chain when enabled).
//!
//! # Detection: sample-peak, not true-peak
//!
//! The reference limiter oversamples each channel 2× to catch inter-sample
//! peaks (true-peak). VXN1 does **not**: it detects on the base-rate magnitude
//! directly. The output is hard-clamped to ±1, so no *sample* ever exceeds
//! digital full scale regardless; inter-sample (reconstructed-waveform) overshoot
//! is left to the host / DAC chain, which owns final true-peak compliance. This
//! is an internal master *safety* limiter, not a true-peak mastering meter, so
//! the 2× interpolators (their 8-tap convolution dominated the per-sample cost)
//! are dropped.
//!
//! # Algorithm
//!
...
use crate::{DelayLine, ms_to_samples, one_pole_coeff};

// ── Sliding-maximum peak detector ───────────────────────────────────────────

/// Sliding-maximum detector over a configurable window of oversampled absolute
/// values.
///
...
impl PeakWindow {
    ...
    /// Push one (oversampled) sample, advancing the window. Stores the absolute
    /// value and updates the monotonic deque in O(1) amortised.
    #[inline]
...
/// with [`process`](Self::process); the engine calls it in the per-sample FX
/// loop when the master limiter is enabled.
