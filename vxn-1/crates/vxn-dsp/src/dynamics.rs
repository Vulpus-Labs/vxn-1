...
//!
//! Ported verbatim from `vxn-2/crates/vxn2-dsp/src/dynamics.rs` (E037 / 0206).
//! The kernel is rate-agnostic — one stereo sample in / out — so it carries no
//! oversampling stage: in VXN2 the 4× interp/decimate lives in the engine, not
//! here. VXN1b runs it at whatever the global OS rate is. Dep adaptation from
//! the VXN2 original: the smoother types come from `crate::smoothing`
//! (re-exported from `vxn-core-utils`) rather than a `crate::smoother` module.

use crate::math::fast_tanh;
use crate::smoothing::{one_pole_coeff, Smoothed};
